/*
  快速排序的基本思想：
    ...
    1、选择一个基准元素（Pivot）： 从待排序数组中选择一个元素作为基准（通常选择第一个元素）。

    2、划分阶段（Partition）：将数组重新排列，使得比基准小的元素位于基准之前，比基准大的元素位于基准之后。基准元素的最终位置就是其在排序后的位置。

    3、递归操作： 递归地对基准左右两侧的子数组进行快速排序。
  ...
  时间复杂度：
    ...
    2、最差时间复杂度：
      在最差情况下，即每次划分都只能将数组分成一个元素和其他元素两部分，快速排序的时间复杂度为 O(n^2)。
      最差情况通常出现在已经有序的数组中，或者在选择基准元素时不幸选择了最大或最小的元素。

    3、平均时间复杂度：
      在平均情况下，快速排序的时间复杂度为 O(n log n)。
      这是通过期望值分析得出的，平均情况下，每个元素都有机会成为基准元素，且划分过程能够均匀地将数组分割。

    4、空间复杂度：
      快速排序是一种原地排序算法，其空间复杂度为 O(log n)。
      这是由于快速排序是通过不断地对数组进行原地划分实现的，而递归调用的栈深度最多为 log n。
  ...
  总结：
    ...
    在实际应用中，快速排序通常是一种性能优秀的排序算法，尤其在大数据集合的排序中表现良好。
    为了减小最差情况的发生概率，通常采用一些优化策略，如随机选择基准元素、三数取中法等。
 */
